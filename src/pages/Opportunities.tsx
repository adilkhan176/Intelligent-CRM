...
import { Target, DollarSign, TrendingUp, Percent, Plus } from "lucide-react";
...
export default function Opportunities() {
  ...
  return (
    <div>
              ...
              <table className="w-full text-sm sm:min-w-[640px]">
                <thead>
                  <tr className="border-b border-hairline text-left text-xs text-ink-muted uppercase tracking-wide">
                    ...
                    <th className="px-4 py-2.5 font-medium text-right hidden sm:table-cell">Delivery Cost</th>
                    <th className="px-4 py-2.5 font-medium text-right hidden sm:table-cell">Margin</th>
                  ...
                  {opportunities.map((lead) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right tabular-nums text-ink-secondary hidden sm:table-cell">{formatCurrency(lead.proposalCost)}</td>
                        <td className="px-4 py-3 text-right tabular-nums hidden sm:table-cell" style={{ color: margin >= 40 ? "var(--success-text)" : "var(--text-secondary)" }}>
                          {margin}%
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className="h-1.5 flex-1 rounded-full bg-surface-3 max-w-24 overflow-hidden">
                              <div
                                ...
                                style={{ width: `${lead.probability}%`, background: "var(--brand)" }}
                              />
                            </div>
      ...
      {showForm && (
        <LeadFormModal mode="opportunity" onClose={() => setShowForm(false)} onCreated={(id) => setSelectedId(id)} />
      )}
    </div>
  );
}
